    ...
    public override void OnActionExecuting(ActionExecutingContext actionContext)
    {
        ...
        if (actionContext.HttpContext.Request.Headers.TryGetValue("ACCESS_TOKEN", out values))
        {
            ...
            { 
                ...
                if (ActionFilter.GetAllActionModel(Action).UserRole == null)
                {
                    checkAction = true;
                }
                else
                {
                    var t = ActionFilter.GetAllActionModel(Action).UserRole!
                        .FirstOrDefault(x => x.ToString().ToLower() == userToken.Role!.ToLower()).ToString();
                    if (t != "0")
                    {
                        checkAction = true;
                    }
